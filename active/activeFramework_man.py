...
def main_loop(data, current_state):
    ...
    '''
    model_feature_s = model_selector('MAN_feature_share', param_handler.dataset_name).to(device=device)
    model_discriminator = model_selector('MAN_discriminator', param_handler.dataset_name).to(device=device)
    model_classifier = model_selector('MAN_classifier', param_handler.dataset_name).to(device=device)
    model_feature_domain_dict = {}

    for domain_name in param_handler.domains:
        model_instance = model_selector('MAN_feature_domain_individual', param_handler.dataset_name).to(device=device)
    ...
    model_instance = model_selector('MAN', param_handler.dataset_name)
...
def train(model_feature_s, model_discriminator, model_classifier, model_feature_domain_dict, loader_dict, optimizer, optimizer_d):
    ...
    '''
    Train the discriminator D independently at first.
    '''
    ...
    while not load_all_batches:
        ...
        for domain_name in param_handler.domains:
            ...
            d_pred = model_discriminator(model_feature_s(x))
            loss_discriminator = loss_domain(d_pred, d)
    ...
    '''
    Train fs fd clf and D after training D independently.
    '''

    load_all_batches = False
    finished_set = set()
    trade_off = param_handler.man_tradeoff
    iters_list = [iter(loader) for loader in loader_dict.values()]
    iter_loader_dict = dict(zip(loader_dict.keys(),iters_list))
    while not load_all_batches:
        # train on each dataset 
        ...
        for domain_name in param_handler.domains:
            ...
            labeled_set_loss, _ = labeled_set_ce_loss(y_pred, y, l)
            
            d_pred = model_discriminator(shared_feature)
            loss_discriminator = loss_domain(d_pred, d)

            loss = labeled_set_loss + loss_discriminator * trade_off
            loss.backward()
